	...
	const std::basic_string<E,T,A> &d2)
{
	bool ret = false;
	size_t pos;
	while(std::basic_string<E,T,A>::npos != (pos = s.find(d1)) ){
...
template<class E, class T, class A>
const std::basic_string<E,T,A> replace(
	std::basic_string<E,T,A> const &s,
	const std::basic_string<E,T,A> &d1, 
	const std::basic_string<E,T,A> &d2)
{
	std::basic_string<E,T,A> dest = s;
	size_t pos;
	while(std::basic_string<E,T,A>::npos != (pos = dest.find(d1)) ){
		dest.replace(pos, d1.length(), d2);
	}
	return dest;
}
...
inline const std::string replace(std::string const &src, const char *d1, const char *d2){
	return replace(src, std::string(d1), std::string(d2));
}
inline const std::wstring replace(std::wstring const &src, const wchar_t *d1, const wchar_t *d2){
	return replace(src, std::wstring(d1), std::wstring(d2));
}
